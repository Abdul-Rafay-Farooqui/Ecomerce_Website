...
import { empty, decQty, incQty, removeFromCart } from '../redux/reducers/cartActions';
  ...
  return (
    <Grid container>
      <Grid item xs={12} lg={cartItems.length > 0 ? 8 : 12} px={2} >
        <CartPage sx={{backgroundColor:"#242424" ,my:"2rem", mx:"auto", p:"2rem", borderRadius:"5px" }}>
          <Typography fontSize={{lg:"h1",xs:"h3"}} color={"white"} >Your Shopping Cart</Typography>
          <Grid container >
            ...
            cartItems.map(item => (
              <ListItem key={item.id} sx={{ borderBottom:"solid", width:"100%", padding: 0 }}>
                ...
                <Grid item xs={3} sx={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                 <Add onClick={() => handleAdd(item)} sx={{ backgroundColor: "#fff", color: "#242424", borderRadius: "5px", ":hover": { color: "white", bgcolor: "black" } }} />
                  <Typography sx={{ px: { xs: "2px", lg: "1rem" }, py: "0.5rem", fontSize: { xs: "12px", lg: "18px" } }}>{item.quantity}</Typography>
                  <Remove onClick={() => handleRemove(item.id, item.quantity)} sx={{ backgroundColor: "#fff", color: "#242424", borderRadius: "5px", ":hover": { color: "white", bgcolor: "black" } }} />
                  </Grid>
      ...
      <Grid item xs={12} lg={4} pr={2} mx={"auto"}>
        <CartPage sx={{ backgroundColor:"#242424" ,my:"2rem", p:"2rem", borderRadius:"5px", marginLeft:"1rem", maxWidth:{sm: "50%", lg: "100%"} }}>
          <Typography variant="h2" color={"white"} >Checkout</Typography>
          <List>
            <ListItem>
              <Grid container>
